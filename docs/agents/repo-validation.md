...
lastReviewedNote: "Reviewed for Toolkit #219: focused ILCD non-flow reference regression covers Other parameter, Functional unit and Production period with Mercury first; full import validation waits for the exact spec candidate."
related:
  - ../../AGENTS.md
  - ../../.docpact/config.yaml
...
| import | all supported format fixtures; native target validation; ILCD non-flow quantitative-reference regression with an elementary pollutant first and absent process type; deterministic package/mapping/bundle hashes; malformed/unsupported input, cancellation, budget, atomic publication | large exchange/issue-spool fixture with wall time/RSS and cross-root determinism |
